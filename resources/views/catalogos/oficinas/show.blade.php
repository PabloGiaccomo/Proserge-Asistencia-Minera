...
@section('title', 'Oficina - Detalle')

@section('content')
@php
    $permissions = session('user.permissions', []);
    $canEdit = \App\Support\Rbac\PermissionMatrix::allowsDirect($permissions, 'oficinas', 'editar');
    $canDelete = \App\Support\Rbac\PermissionMatrix::allowsDirect($permissions, 'oficinas', 'eliminar');
@endphp
<div class="page-header">
    <div class="page-header-content">
        <div>
            <h1 class="page-title">Detalle de Oficina</h1>
            <p class="page-subtitle">{{ $item['nombre'] ?? '' }}</p>
        </div>
        <div class="page-actions">
            @if($canEdit)
                <a href="{{ route('catalogos.oficinas.edit', $item['id']) }}" class="btn btn-primary">Editar</a>
            @endif
            @if($canDelete)
            ...
            @endif
            <a href="{{ route('catalogos.oficinas.index') }}" class="btn btn-outline">Volver</a>
        </div>
    </div>
</div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="detail-grid">
                ...
                <div class="detail-item"><span class="detail-label">Estado</span><span class="badge badge-{{ ($item['activo'] ?? true) ? 'success' : 'danger' }}">{{ ($item['activo'] ?? true) ? 'Activa' : 'Inactiva' }}</span></div>
            </div>
        </div>
    </div>
</div>
