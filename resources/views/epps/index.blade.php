...
>
    ...
    <section class="epp-panel">
        ...
        <form method="GET" action="{{ route('epps.index') }}" class="epp-filter-grid">
            ...
            <div class="epp-filter-actions">
                ...
                <a href="{{ route('epps.index') }}" class="epp-btn epp-btn-light">Limpiar</a>
            </div>
        </form>
    </section>

    @if(false && $canUpdateEpps)
    <div class="epp-grid">
        <section class="epp-panel">
            <div class="epp-panel-header">
                <div>
    ...
    <section class="epp-panel">
        ...
        @if($entregasIsPaginator && $entregas->total() > 0)
            @php
                $paginationPages = collect([
                    1,
                    $entregas->currentPage() - 1,
                    $entregas->currentPage(),
                    $entregas->currentPage() + 1,
                    $entregas->lastPage() - 1,
                    $entregas->lastPage(),
                ])
                    ->filter(fn ($page) => $page >= 1 && $page <= $entregas->lastPage())
                    ->unique()
                    ->sort()
                    ->values();
            ...
            <div class="epp-pagination">
                <p class="epp-pagination-summary">
                    Mostrando {{ $entregas->firstItem() }} - {{ $entregas->lastItem() }} de {{ $entregas->total() }} registros
                </p>
